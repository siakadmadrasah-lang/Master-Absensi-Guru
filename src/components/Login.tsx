  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-emerald-950 to-slate-900 text-slate-100 flex flex-col justify-center items-center py-3 px-3 sm:py-5 sm:px-4 lg:py-6 lg:px-6">
      ...
      <div className="relative w-full max-w-3xl grid grid-cols-1 lg:grid-cols-12 gap-0 bg-slate-900/90 backdrop-blur-xl border border-emerald-500/30 rounded-2xl shadow-2xl overflow-hidden my-auto">
        ...
        {/* Left Side: Madrasah Identity & Info */}
        <div className="lg:col-span-5 bg-gradient-to-b from-emerald-800 to-emerald-950 p-4 sm:p-5 flex flex-col justify-between border-b lg:border-b-0 lg:border-r border-emerald-700/50">
          <div>
            {/* Dynamic Application / Madrasah Logo (Centered above the Madrasah Name - Circular Shape) */}
            <div className="flex items-center justify-center gap-3 mb-3.5 pt-1">
              {(profile.appLogoUrl || profile.logoUrl) ? (
                <div className="w-16 h-16 rounded-full bg-white flex items-center justify-center shadow-lg border-2 border-emerald-400/60 shrink-0 overflow-hidden ring-4 ring-emerald-500/20 transition-transform duration-300 hover:scale-105">
                  <img 
                ...
                <div className="w-14 h-14 rounded-full bg-white/10 flex items-center justify-center shadow-lg border border-white/20 shrink-0 ring-4 ring-emerald-500/20 overflow-hidden">
                  <div className="w-full h-full bg-emerald-950/90 rounded-full flex items-center justify-center">
                    <Building2 className="w-7 h-7 text-emerald-300" />
                  </div>
                </div>
              )}

              {profile.secondaryLogoUrl && !profile.appLogoUrl && (
                <div className="w-16 h-16 rounded-full bg-white flex items-center justify-center shadow-lg border-2 border-emerald-400/60 shrink-0 overflow-hidden ring-4 ring-emerald-500/20 transition-transform duration-300 hover:scale-105">
                  <img 
            ...
            {/* Madrasah Name & SIMPRESENSI GTK with Fingerprint Icon on Left */}
            <div className="flex items-center space-x-2.5 mb-3 sm:mb-4">
              <div className="w-9 h-9 rounded-lg bg-white/10 flex items-center justify-center border border-white/20 shadow-inner shrink-0">
              ...
              <div className="min-w-0 flex-1">
                ...
                <h1 className="text-base sm:text-lg font-bold text-white tracking-tight leading-tight line-clamp-2">
                  {profile.name}
                </h1>
              </div>
            </div>

            <div className="bg-emerald-900/60 border border-emerald-600/40 rounded-lg p-2.5 mb-3 space-y-1 text-[11px]">
              <div className="flex items-center text-emerald-200">
                <Building2 className="w-3.5 h-3.5 mr-1.5 text-emerald-400 shrink-0" />
                <span className="font-semibold truncate">{profile.letterHeader1 || "LP Ma'arif NU"}</span>
              </div>
              <div className="text-emerald-300/80 pl-5">
                NSM: {profile.nsm} | NPSN: {profile.npsn}
              </div>
              <div className="text-emerald-300/70 pl-5 truncate">
                {profile.village}, {profile.district}, {profile.city}
              </div>
            </div>

            <div className="space-y-2">
              <h3 className="text-[10px] font-bold text-emerald-200 uppercase tracking-wider">
                Fitur Unggulan Sistem:
              </h3>
              <div className="flex items-start space-x-2 text-[11px] text-emerald-100/90">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0 mt-0.5" />
                <span>Autentikasi NIK & PIN Resmi GTK</span>
              </div>
              <div className="flex items-start space-x-2 text-[11px] text-emerald-100/90">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0 mt-0.5" />
                <span>Cetak PDF Jam Berangkat & Pulang</span>
              </div>
              <div className="flex items-start space-x-2 text-[11px] text-emerald-100/90">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0 mt-0.5" />
                <span>Format SPTJM Kemenag & Simpatika</span>
              </div>
              <div className="flex items-start space-x-2 text-[11px] text-emerald-100/90">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0 mt-0.5" />
                <span>Simulasi Mesin Fingerprint & Face Scan</span>
              </div>
            </div>
          </div>

          <div className="mt-4 pt-2.5 border-t border-emerald-700/60 text-[10px] text-emerald-300/70 flex items-center justify-between">
